...
// Hide/Show the page with key
- (BOOL) setPageHidden:(BOOL)hidden forPageWithKey:(NSString*)key;
...
/**
 * Display notifications on the display
 *
 * @param show popups TRUE
 *
 * @return <c>TRUE</c> if the value is sent, otherwise <c>FALSE</c>
 */
- (BOOL) setNotificationDisplay:(BOOL) enabled;
- (BOOL) getNotificationDisplay;
